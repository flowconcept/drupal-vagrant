...
if [ -n "$SITE" ]; then
  ...
  \$sites['localhost'] = '$SITE';" > /vagrant/web/sites/sites-local.php

  if [ -d "/vagrant/web/sites/$SITE" ]; then
    cp /vagrant/web/sites/default/settings.local.php "/vagrant/web/sites/$SITE/settings.local.php"
  fi
fi
